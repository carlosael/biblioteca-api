...
const cadastrarAutor = async (req, res) => {
    ...
    try {
        ...
        if (autor.rowCount === 0) {
            return res.status(400).json('Não possível cadastrar o autor.')
        }
...
const excluirAutor = async (req, res) => {
    ...
    try {
        ...
        const queryTwo = `
        select l.id, a.id as autor_id, a.nome from livros l
        join autores a on l.autor_id = a.id
        where autor_id = $1
        `
        const autorTemLivro = await conexao.query(queryTwo, [id]);

        if (autorTemLivro.rowCount === 0) {
            ...
            return res.status(404).json('Autor não pode ser excluído.')
        }
